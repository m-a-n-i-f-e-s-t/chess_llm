# From torch.distributed.nn.functional
import torch
...
class _AllGather(Function):
    ...
    # Fix for https://github.com/pytorch/pytorch/issues/58005#issuecomment-1778029156
    @staticmethod
    def backward(ctx, *grad_outputs):
        if dist.get_backend(group=ctx.group) is dist.Backend.NCCL:
            rank = dist.get_rank(group=ctx.group)
            gx = torch.empty_like(grad_outputs[rank])
            gx = _Reduce_Scatter.apply(ReduceOp.SUM, ctx.group, gx, *grad_outputs) / dist.get_world_size(group=ctx.group)
        else:
            ...
            gx = torch.sum(torch.stack(gxs), dim=0) / dist.get_world_size(group=ctx.group)
        return (None, gx)
...
class _AllReduce(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if torch.isnan(grad_output).any():
            print(f"grad_output is nan at rank {rank}")
            torch.distributed.breakpoint(rank)
        grad_in = _AllReduce.apply(ctx.op, ctx.group, grad_output)
        if torch.isnan(grad_in).any():
            print(f"grad_in is nan at rank {rank}")
            torch.distributed.breakpoint(rank)
        grad_in = grad_in / world_size
        if torch.isnan(grad_in).any():
            print(f"grad_in is nan at rank {rank} after normalization")
            torch.distributed.breakpoint(rank)
